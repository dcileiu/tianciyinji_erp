...
| -------------- | ------ | ---- | ---- |
| user\_id       | uuid   | true | 用户ID |
| resource\_code | string | true | 资源代码 |

响应参数:

| 参数名             | 参数类型    | 描述    |
| --------------- | ------- | ----- |
| has\_permission | boolean | 是否有权限 |
...
| parent\_id  | uuid    | false | 父菜单ID |
| sort\_order | integer | false | 排序    |
...
| 参数名      | 参数类型 | 是否必需 | 描述   |
| -------- | ---- | ---- | ---- |
| role\_id | uuid | true | 角色ID |
| menu\_id | uuid | true | 菜单ID |
  ...
  const getUserMenus = async () => {
    ...
    try {
      const { data, error } = await supabase
        .rpc('get_user_menus', { p_user_id: user.value.id })

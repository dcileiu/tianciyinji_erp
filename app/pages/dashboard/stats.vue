    ...
    <div class="grid gap-4 lg:grid-cols-2">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>{{ getPeriodLabel() }}的销售数据变化</CardDescription>
        </CardHeader>
        <CardContent>
          <div
          ...
          >
            <div class="text-center space-y-3">
              <LineChart class="mx-auto h-12 w-12 text-muted-foreground" />
              <div>
          ...
          >
            <div class="text-center space-y-3">
              ...
              <div>
                ...
                <p class="text-xs text-muted-foreground">显示不同状态订单的分布情况</p>
              </div>
            </div>
          </div>
        </CardContent>
          ...
          <div class="space-y-4">
            ...
            >
              ...
              <div class="text-right">
                ...
                <p class="text-xs text-muted-foreground">¥{{ formatCurrency(product.revenue) }}</p>
              </div>
            </div>
          </div>
        </CardContent>
          ...
          <div class="space-y-4">
            <div class="flex items-center justify-between p-3 rounded-lg bg-muted/50">
              <div class="space-y-1">
                ...
                <p class="text-xs text-muted-foreground">{{ getPeriodLabel() }}新注册</p>
              </div>
              <div class="text-right">
                <p class="text-lg font-bold text-green-600">+{{ newCustomers }}</p>
                <p class="text-xs text-muted-foreground">较上期 +{{ customerGrowth }}%</p>
              </div>
            </div>
            <div class="flex items-center justify-between p-3 rounded-lg bg-muted/50">
              <div class="space-y-1">
                <p class="text-sm font-medium">活跃客户</p>
                <p class="text-xs text-muted-foreground">{{ getPeriodLabel() }}有交易</p>
              </div>
              <div class="text-right">
                <p class="text-lg font-bold text-blue-600">{{ activeCustomers }}</p>
                <p class="text-xs text-muted-foreground">活跃率 {{ activityRate }}%</p>
              </div>
            </div>
            <div class="flex items-center justify-between p-3 rounded-lg bg-muted/50">
              <div class="space-y-1">
              ...
              <div class="text-right">
                <p class="text-lg font-bold text-purple-600">{{ retentionRate }}%</p>
                <p class="text-xs text-muted-foreground">较上期 +{{ retentionChange }}%</p>
              </div>
            </div>
          </div>
        </CardContent>
          ...
          >
            <div class="text-center space-y-3">
              ...
              <div>
                ...
                <p class="text-xs text-muted-foreground">各地区销售额分布热力图</p>
              </div>
            </div>
          </div>
          <div class="space-y-4">
            ...
            >
              ...
              <div class="text-right space-x-4">
                <span class="text-sm font-medium">¥{{ formatCurrency(region.sales) }}</span>
...
const totalRevenue = ref(2345678)
...
// 销售排行榜数据
const topSales = ref([
  { id: 1, name: '张三', department: '华东区', amount: 156800, orders: 45 },
  { id: 2, name: '李四', department: '华南区', amount: 143200, orders: 38 },
  { id: 3, name: '王五', department: '华北区', amount: 128900, orders: 42 },
  { id: 4, name: '赵六', department: '西南区', amount: 115600, orders: 35 },
  { id: 5, name: '钱七', department: '东北区', amount: 98400, orders: 28 },
])

// 热销产品数据
const topProducts = ref([
  { id: 1, name: '智能手机 Pro', category: '电子产品', sales: 245, revenue: 588000 },
  { id: 2, name: '无线耳机', category: '配件', sales: 189, revenue: 132300 },
  { id: 3, name: '平板电脑', category: '电子产品', sales: 156, revenue: 468000 },
  { id: 4, name: '智能手表', category: '可穿戴', sales: 134, revenue: 201000 },
  { id: 5, name: '充电宝', category: '配件', sales: 98, revenue: 29400 },
])

// 地域数据
const regionData = ref([
  { name: '华东地区', sales: 856200, percentage: 35.2, color: '#3b82f6' },
  { name: '华南地区', sales: 642800, percentage: 26.4, color: '#10b981' },
  { name: '华北地区', sales: 523400, percentage: 21.5, color: '#f59e0b' },
  { name: '西南地区', sales: 298600, percentage: 12.3, color: '#ef4444' },
  { name: '东北地区', sales: 114500, percentage: 4.7, color: '#8b5cf6' },
])
...
const refreshStats = () => {
  // 模拟数据刷新
  totalRevenue.value = Math.floor(Math.random() * 1000000) + 2000000

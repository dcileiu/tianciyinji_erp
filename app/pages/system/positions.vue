<template>
  <div class="space-y-6">
    ...
    <Card>
      ...
      <CardContent>
        <div class="flex justify-center py-8" v-if="loading">
          <div
    ...
    <Dialog v-model:open="showCreateDialog">
      <DialogContent class="max-w-2xl">
        <DialogHeader>
          <DialogTitle
            >{{ editingPosition ? "编辑岗位" : "新增岗位" }}</DialogTitle
          >
        </DialogHeader>
...
<script lang="ts" setup>
  ...
  const savePosition = () => {
    if (editingPosition.value) {
      // 更新现有岗位
      const index = mockPositions.value.findIndex(
        (p) => p.id === editingPosition.value.id
      );
      if (index !== -1) {
        const department = departments.value.find(
          (d) => d.id === positionForm.value.department_id
        );

  ...
  <div>
    ...
    <Card class="mb-6">
      <CardContent>
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <Button class="w-full" variant="outline" @click="resetFilters">
        ...
        <Table>
          ...
          <TableBody>
            <TableRow v-if="filteredDictionaries.length === 0">
              <TableCell class="text-center py-8" colspan="6">
            ...
            >
              ...
              <TableCell>
                <Badge
                  :variant="
                    dictionary.status === 'active' ? 'default' : 'destructive'
                  "
                >
    ...
    <Dialog v-model:open="showCreateDialog">
      <DialogContent class="max-w-2xl">
        <DialogHeader>
          <DialogTitle
            >{{ editingDictionary ? "编辑字典" : "新建字典" }}</DialogTitle
          >
        </DialogHeader>
    ...
    <Dialog v-model:open="showItemsDialog">
      <DialogContent class="max-w-4xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle
            >管理字典项目 - {{ selectedDictionary?.name }}</DialogTitle
          >
        </DialogHeader>
        <div class="space-y-4">
          ...
          <div class="space-y-3">
            ...
            >
              <div class="space-y-2">
                <Label>值</Label>
                <Input placeholder="项目值" v-model="item.value" />
              </div>

              <div class="space-y-2">
                <Label>标签</Label>
                <Input placeholder="显示标签" v-model="item.label" />
              </div>

              <div class="space-y-2">
                <Label>排序</Label>
                <Input max="999" min="1" type="number" v-model="item.sort" />
              </div>
        ...
        <DialogFooter>
          <Button variant="outline" @click="showItemsDialog = false"
            >取消</Button
          >
  ...
  const saveDictionary = async () => {
    try {
      ...
      if (editingDictionary.value) {
        // 更新字典
        const index = dictionaries.value.findIndex(
          (d) => d.id === currentDictionary.value.id
        );
  ...
  const saveDictionaryItems = async () => {
    try {
      ...
      if (selectedDictionary.value) {
        const index = dictionaries.value.findIndex(
          (d) => d.id === selectedDictionary.value?.id
        );
        if (index !== -1 && dictionaries.value[index]) {

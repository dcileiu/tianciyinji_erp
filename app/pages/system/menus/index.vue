...
const saveMenu = async (formData: MenuForm) => {
  ...
  try {
    if (editingMenu.value && editingMenu.value.id) {
      const { error: updateError } = await updateMenu(editingMenu.value.id, formData)
      if (updateError) {
...
const updateSelectAllState = () => {
  const allIds = getAllMenuIds(menus.value)
  selectAll.value = allIds.length > 0 && selectedIds.value.length === allIds.length
}
...
const confirmBatchDelete = () => {
  if (window.confirm(`确定要删除选中的 ${selectedIds.value.length} 个菜单吗？`)) {
    batchDelete()
  }
}

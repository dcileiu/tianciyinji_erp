<script lang="ts" setup>
  ...
  const { items, loading, list, create, update, remove } =
    useMasterEntity<OrderRow>("/api/purchase/orders");
  ...
  const openEdit = async (row: OrderRow) => {
    try {
      const detail = await $fetch<{
        ...
        data: OrderRow;
        message: string;
      }>("/api/purchase/orders", { query: { id: row.id } });
  ...
  const supplierName = (id: string) =>
    suppliers.value.find((c) => c.id === id)?.name || id;
...
<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-3xl font-bold tracking-tight">采购订单</h1>
        <p class="text-muted-foreground">创建、确认与跟踪采购订单</p>
      </div>
        ...
        <table class="w-full text-sm">
          ...
          <tbody>
            ...
            >
              ...
              <td class="px-4 py-3">{{ supplierName(row.supplier_id) }}</td>
          ...
          <div class="grid gap-4 md:grid-cols-2">
            <div class="space-y-2">
              ...
              <Select v-model="form.supplier_id">
                ...
                <SelectContent>
                  <SelectItem v-for="c in suppliers" :key="c.id" :value="c.id">
                    {{ c.name }}
                  </SelectItem>
                </SelectContent>
              </Select>

...
export const useDbInit = () => {
  ...
  const updateStepStatus = (
    ...
    if (step) {
      ...
      initializationProgress.value = Math.round(
  ...
  const withRetry = async <T>(
    ...
    for (let attempt = 1; attempt <= config.maxRetries; attempt++) {
      try {
        addLog(
          "info",
          `执行 ${operationName} (尝试 ${attempt}/${config.maxRetries})`
        );
  ...
  const validateAdminPermissions = async (userId: string): Promise<boolean> => {
    try {
      ...
      if (missingPermissions.length > 0) {
        addLog(
          "warning",
          `管理员缺少以下权限: ${missingPermissions.join(", ")}`
        );
        return false;
      }
  ...
  const checkTables = async () => {
    try {
      ...
      for (let i = 0; i < requiredTables.length; i++) {
        ...
        const progress =
          Math.round(((i + 1) / requiredTables.length) * 80) + 20;
  ...
  // 创建/绑定管理员：使用当前登录用户，不再硬编码账号密码
  const createAdmin = async () => {
    try {
      updateStepStatus("create-admin", "running", 60);
      addLog("info", "绑定当前登录用户为管理员...");

      const currentUserData = getCurrentUser();
      if (!currentUserData?.id) {
        addLog(
          "error",
          "请先登录后再初始化，或在 Supabase SQL 中执行 scripts/assign-admin-permissions.sql"
        );
      ...
      updateStepStatus("create-admin", "error", 0);
      addLog("error", `绑定管理员失败: ${error.message}`);
      return { success: false, userId: null };
    }
  };
  ...
  const initializeAll = async () => {
    try {
      ...
      if (!canInitialize.value) {
        ...
        addLog(
          "info",
          "💡 如需在生产环境初始化，请设置环境变量 VITE_ALLOW_DB_INIT=true"
        );
        return false;
      }
      ...
      addLog(
        "info",
        "📋 初始化步骤: 连接检查 → 表结构验证 → 角色检查 → 管理员创建 → 权限分配"
      );
      ...
      const finalValidation = await validateAdminPermissions(
        adminResult.userId!
      );
      ...
      addLog("info", "📝 管理员账号已就绪", {
        note: "请使用初始化时配置的账号登录；密码不会写入日志",
      });
      ...
      addLog(
        "info",
        "💡 建议检查: 1) Supabase 连接配置 2) 数据库迁移状态 3) RLS 策略设置"
      );
      initializationProgress.value = 0;
      return false;
    } finally {
  ...
  return {
    ...
    hasWarnings: computed(() =>
      logs.value.some((log) => log.level === "warning")
    ),
    ...
    getInitializationSummary: () => ({
      ...
      completedSteps: steps.value.filter((s) => s.status === "completed")
        .length,

...
export const useWorkshops = () => {
  ...
  const workshopStats = computed(() => {
    ...
    // 计算平均产能利用率
    const totalUtilization = workshops.value.reduce((sum, ws) => sum + (ws.capacity_utilization || 0), 0)
    ...
    const activeEquipment = workshops.value.reduce((sum, ws) => sum + (ws.active_equipment_count || 0), 0)
  ...
  const createWorkshop = async (workshopData: WorkshopInsert) => {
    try {
      ...
      // 生成车间编码
      const workshopCode = await generateWorkshopCode(workshopData.workshop_type || 'other')
  ...
  const getEquipmentStatus = (workshop: Workshop) => {
    ...
    const active = workshop.active_equipment_count || 0

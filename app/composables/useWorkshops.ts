...
export const useWorkshops = () => {
  ...
  const workshopStats = computed(() => {
    ...
    // 计算平均产能利用率
    const totalUtilization = workshops.value.reduce(
      (sum, ws) => sum + (ws.utilization || 0),
      0
    );
    const avgUtilization = total > 0 ? Math.round(totalUtilization / total) : 0;

    // 计算总设备数和活跃设备数
    const totalEquipment = workshops.value.reduce(
      (sum, ws) => sum + (ws.equipment_count || 0),
      0
    );
    const activeEquipment = workshops.value.reduce(
      (sum, ws) => sum + (ws.equipment_count || 0),
      0
    );
  ...
  // 更新设备数量
  const updateEquipmentCount = async (
    id: string,

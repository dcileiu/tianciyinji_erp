...
export const useUserStore = defineStore('user', {
  ...
  getters: {
    ...
    displayName: (state): string => {
      return state.profile?.name || state.profile?.username || state.authUser?.email?.split('@')[0] || '未知用户'
    },
    ...
    userAvatar: (state): string => {
      return state.profile?.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${state.authUser?.id}`
    },
    ...
    hasPermission: (state) => (permission: string): boolean => {
      ...
      if (state.roles.includes('super_admin')) return true // 超级管理员拥有所有权限

import { desc, eq } from "drizzle-orm";
import { db } from "../../../db";
import { financeInvoices } from "../../../db/schema/ops";
import { generateOrderNo, handleApiError } from "../../_utils/crud";
import { assertPermission } from "../../_utils/permissions";

function toApi(row: typeof financeInvoices.$inferSelect) {
  return {
    id: row.id,
    invoice_no: row.invoiceNo,
    direction: row.direction,
    party_id: row.partyId,
    amount: row.amount,
    invoice_date: row.invoiceDate,
    status: row.status,
    remark: row.remark,
    created_at: row.createdAt,
    updated_at: row.updatedAt,
  };
}

export default defineEventHandler(async (event) => {
  const method = getMethod(event);
  try {
    await assertPermission(event, "invoice:view");
    if (method === "GET") {
      const rows = await db
        .select()
        .from(financeInvoices)
        .orderBy(desc(financeInvoices.createdAt));
      return { code: 0, message: "获取成功", data: rows.map(toApi) };
    }
    if (method === "POST") {
      const body = await readBody(event);
      if (!(body?.direction && body?.party_id)) {
        throw createError({
          statusCode: 400,
          statusMessage: "方向和往来方不能为空",
        });
      }
      const [created] = await db
        .insert(financeInvoices)
        .values({
          invoiceNo: body.invoice_no || generateOrderNo("INV"),
          direction: body.direction,
          partyId: body.party_id,
          amount: String(body.amount || 0),
          invoiceDate: body.invoice_date || null,
          status: body.status || "issued",
          remark: body.remark || null,
        })
        .returning();
      return { code: 0, message: "创建成功", data: toApi(created) };
    }
    if (method === "PUT") {
      const body = await readBody(event);
      if (!body?.id) {
        throw createError({ statusCode: 400, statusMessage: "ID 不能为空" });
      }
      const patch: Record<string, unknown> = { updatedAt: new Date() };
      if (body.direction !== undefined) {
        patch.direction = body.direction;
      }
      if (body.party_id !== undefined) {
        patch.partyId = body.party_id;
      }
      if (body.amount !== undefined) {
        patch.amount = String(body.amount);
      }
      if (body.invoice_date !== undefined) {
        patch.invoiceDate = body.invoice_date;
      }
      if (body.status !== undefined) {
        patch.status = body.status;
      }
      if (body.remark !== undefined) {
        patch.remark = body.remark;
      }
      const [updated] = await db
        .update(financeInvoices)
        .set(patch as any)
        .where(eq(financeInvoices.id, body.id))
        .returning();
      return { code: 0, message: "更新成功", data: toApi(updated) };
    }
    if (method === "DELETE") {
      const body = await readBody(event);
      await db.delete(financeInvoices).where(eq(financeInvoices.id, body.id));
      return { code: 0, message: "删除成功" };
    }
    throw createError({ statusCode: 405, statusMessage: "Method not allowed" });
  } catch (error: unknown) {
    return handleApiError(error);
  }
});
